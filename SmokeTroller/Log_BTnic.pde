...
*/
  ...
  byte chkCmd() {
    ...
    if(cmdBuffer[0] == CMD_GET_BOIL
	  ...
	  else if (cmdBuffer[0] == CMD_STEAM) logFieldI(flowRate[PIT_2]);
	  #else
    ...
    //1 Param, HLT-Kettle
      ...
      byte smokerPit = getCmdParamNum(1);
      if (getCmdParamCount() != 1 || smokerPit > PIT_3) return CMD_REJECT_PARAM;
      cmdBufLen = 1; //Reuse CMD code
      logFieldI(smokerPit);
      if (cmdBuffer[0] == CMD_GET_CAL) {
        for (byte i = 0; i < 10; i++) {
            logFieldI(calibVols[smokerPit][i]);
            logFieldI(calibVals[smokerPit][i]);
        }
      } 
      else if (cmdBuffer[0] == CMD_GET_VSET) {
        logFieldI(getCapacity(smokerPit));
        logFieldI(getVolLoss(smokerPit));  
      } 
      else if (cmdBuffer[0] == CMD_VOL) {
        logFieldI(volAvg[smokerPit]);
        #ifdef FLOWRATE_CALCS
          logFieldI(flowRate[smokerPit]);
        #else
          logFieldI(0);
        #endif
      } 
      else if (cmdBuffer[0] == CMD_TEMP) logFieldI(temp[smokerPit]);
      logEnd();
    } 
    ...
    //1 Param, HLT-Steam
      ...
      if (cmdBuffer[0] == CMD_GET_OSET) {
        ...
        byte pct;
        if (PIDEnabled[smokerPit]) pct = PIDOutput[smokerPit] / PIDCycle[smokerPit];
        else if (heatStatus[smokerPit]) pct = 100;
        else pct = 0;
        logFieldI(pct);
      } 
      else if (cmdBuffer[0] == CMD_SETPOINT) logFieldI(setpoint[smokerPit] / 100);
      logEnd();
    } 
    ...
    //Unique Commands
      ...
      byte smokerPit = getCmdParamNum(1);
      if (getCmdParamCount() != 9 || smokerPit > PIT_3) return CMD_REJECT_PARAM;
      setPIDEnabled(smokerPit, getCmdParamNum(2));
      setPIDCycle(smokerPit, getCmdParamNum(3));
      setPIDp(smokerPit, getCmdParamNum(4));
      setPIDi(smokerPit, getCmdParamNum(5));
      setPIDd(smokerPit, getCmdParamNum(6));
      if (smokerPit == PIT_3) {
        ...
        setHysteresis(smokerPit, getCmdParamNum(7));
      }
      sendOK();
    } 
    ...
    //One Param, Program Number
      ...
      byte smokerPit = getCmdParamNum(1);
      if (getCmdParamCount() != 21 || smokerPit > PIT_3) return CMD_REJECT_PARAM;
      for (byte i = 0; i < 10; i++) setVolCalib(smokerPit, i, getCmdParamNum(i * 2 + 3), getCmdParamNum(i * 2 + 2));
      sendOK();
    } 
    else if (cmdBuffer[0] == CMD_SET_VSET) {
      byte smokerPit = getCmdParamNum(1);
      if (getCmdParamCount() != 3 || smokerPit > PIT_3) return CMD_REJECT_PARAM;
      setCapacity(smokerPit, getCmdParamNum(2));
      setVolLoss(smokerPit, getCmdParamNum(3));
      sendOK();
    } 
      ...
      byte smokerPit = getCmdParamNum(1);
      if (getCmdParamCount() != 2 || smokerPit > PIT_3) return CMD_REJECT_PARAM;
      setSetpoint(smokerPit, getCmdParamNum(2));
      sendOK();
    } 
    else if (cmdBuffer[0] == CMD_SET_TIMRERSTATUS || cmdBuffer[0] == CMD_SET_TIMERVALUE) {
      byte timer = getCmdParamNum(1);
      if (getCmdParamCount() != 2 || timer > TIMER_S2) return CMD_REJECT_PARAM;
      ...
      byte timer = getCmdParamNum(1);
      if (getCmdParamCount() != 1 || timer > TIMER_S2) return CMD_REJECT_PARAM;
      logFieldI(timer);
