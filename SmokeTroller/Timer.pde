...
void updateTimers() {
  for (byte timer = PIT_1; timer <= PIT_3; timer++) {
    if (timerStatus[timer]) {
      ...
      if (timerValue[timer] > now - lastTime[timer]) {
        timerValue[timer] -= now - lastTime[timer];
          ...
          logStart_P(LOGDEBUG);
          if(timer == PIT_1) logField("TIMER_S1 has expired");
          else if (timer == PIT_2) logField("TIMER_S2 has expired");
          else logField("TIMER_S3 has expired");
          logEnd();
        #endif
        timerValue[timer] = 0;
        timerStatus[timer] = 0;
