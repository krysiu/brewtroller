...
void updateTimers() {
  for (byte timer = TIMER_S1; timer <= TIMER_S3; timer++) {
    if (timerStatus[timer]) {
      ...
      if (timerValue[timer] > now - lastTime[timer]) {
        timerValue[timer] -= now - lastTime[timer];
          ...
          logStart_P(LOGDEBUG);
          if(timer == TIMER_S1)
            logField("TIMER_S1 has expired");
          else if(timer == TIMER_S2)
            logField("TIMER_S2 has expired");
          else 
            logField("TIMER_S3 has expired");
          logEnd();
        #endif
        timerValue[timer] = 0;
        timerStatus[timer] = 0;
