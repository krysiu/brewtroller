...
void utility::strLPad(char retString[], uint8_t len, char pad) {
  char strVal[len + 1];
...
void utility::vftoa(unsigned long val, char retStr[], unsigned int divisor, uint8_t decimal) {
  char lbuf[11];
  itoa(divisor - 1, lbuf, 10);
  uint8_t precision = strlen(lbuf);
...
void utility::truncFloat(char retStr[], uint8_t len) {
  retStr[len] = '\0';
  if (retStr[len - 1] == '.') retStr[len - 1] = '\0';
}

unsigned long utility::pow10(uint8_t power) {
  unsigned long retValue = 1;
  for (uint8_t i = 0; i < power; i++) retValue *= 10;
  return retValue;
}

OpenTroller::utility OpenTroller::Utility;