...
// create global array of PCintPort objects
//
...
void PCintPort::addPin(uint8_t mode, uint8_t mask, PCIntvoidFuncPtr userFunc)
{
	...
	do 
  {
		if (!p->PCintMode) 
    { 
      // found an available pin data structure
...
void PCintPort::detachInterrupt(uint8_t pin)
{
	...
	PCintPort& port = PCintPort::pcIntPorts[portNum - 1];
